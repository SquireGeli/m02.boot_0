...
class Termometro():
    ...
    def convertir(self,grados,tuUnidad):
        resultado = 0
        if tuUnidad == 'F':
...
class NumberInput():
    ...
    def on_event (self, event):
        if event.type == KEYDOWN:
            if event.unicode.isdigit() and len(self.__strValue) <= 9:
...
class mainApp():
    ...
    def start(self):
        while True:
            ...
            #Pintamos el termometroen su posiciob        
            self.__screen.blit(self.termometro.custome, (50,34))
            
            #Pintamos el cuadro de texto
            text= self.entrada.render() #Obtenemo rectangulo blanco y foto del texto y lo asignamos a text
            pygame.draw.rect(self.__screen,(255, 255, 255), text[0])
            #Creamos el rectang blanco con sus datos (posicion y tamaño) que estan en text[0]
